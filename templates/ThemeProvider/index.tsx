...
const ThemeProvider:React.FC<{children: any}>  = ({children, ...rest}) => {
	const [theme, setTheme] = useState(typeof window !== "undefined" ? document.querySelector("html")?.getAttribute("data-theme") as Theme : "light");
  ...
  useEffect(() => {
    ...
    setTheme(initialColorValue as Theme);
		window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', event => {
			...
			if(!storedTheme)
				changeTheme(newColorScheme, false);
		});
  }, []);
